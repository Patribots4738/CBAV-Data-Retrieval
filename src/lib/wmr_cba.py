...
class cba4:
	...
	def __init__(self, serial_number=None, interface_number=0):
		...
		if serial_number:
			...
			for device in devices:
				...
				# Requested serial number was not found.
				return
	...
	def close(self):
		...
		if (self.__handle_read != -1):
			self.__usb.MPUSBClose(self.__handle_read)
			self.__handle_read = -1
		if (self.__handle_write != -1):
			self.__usb.MPUSBClose(self.__handle_write)
			self.__handle_write = -1
		# end close()
	...
	def __del__(self):
		try:
			...
			# Never allow destructor cleanup failures to propagate.
			pass
		# end __del__
	...
	def get_last_error(self):
		"""
		Returns a human-readable reason for an invalid connection, or None.
		"""
		if self.is_valid():
			return None
		if self.__usb:
			load_error = self.__usb.get_load_error()
			if load_error:
				return load_error
		if self.__handle_read == -1 or self.__handle_write == -1:
			return "Could not open CBA IV USB endpoints. Verify driver install and device connection."
		return "Unknown CBA IV connection error"
		# end get_last_error()
	...
	def get_serial_number(self):
		...
		"""
		config = self.__config_bytes
		if (config is None) or (len(config) < 8):
			return 0
		config = cast(list[int], config)
		b4 = int(config[4])
		b5 = int(config[5])
		b6 = int(config[6])
		b7 = int(config[7])
		return b4 + (b5 * 0x100) + (b6 * 0x10000) + (b7 * 0x1000000)
...
class mpusbapi:
	...
	def __init__(self):
		...
		if sys.platform == "win32":
			dll_paths = [
				Path(__file__).resolve().parent / "mpusbapi.dll",
				Path("mpusbapi.dll"),
			]
			first_error = None
			for dll_path in dll_paths:
				try:
					self.__dll = ctypes.WinDLL(str(dll_path))
					self.__load_error = None
					break
				except OSError as e:
					err = str(e)
					if first_error is None:
						first_error = err
					# Preserve a bitness-mismatch message if we encounter one.
					if (self.__load_error is None) or ("WinError 193" in err):
						self.__load_error = err
			if not self.__dll:
				if self.__load_error is None:
					self.__load_error = first_error
				if self.__load_error and "WinError 193" in self.__load_error:
					self.__load_error += (
						" (This usually means bitness mismatch: 32-bit mpusbapi.dll requires 32-bit Python.)"
					)
				return
		else:
			self.__load_error = "Only Windows is supported by mpusbapi.dll"
			return
		...
		self.__dll._MPUSBOpen.restype = ctypes.c_void_p
		...
		self.__dll._MPUSBRead.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_long,
		...
		self.__dll._MPUSBWrite.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_long,
		                                   ctypes.POINTER(ctypes.c_long), ctypes.c_long]
		self.__dll._MPUSBWrite.restype = ctypes.c_long
		
		# BOOL (*MPUSBClose)(HANDLE handle);
		self.__dll._MPUSBClose.argtypes = [ctypes.c_void_p]
		self.__dll._MPUSBClose.restype = ctypes.c_bool
		# end __init__()
	
	def get_load_error(self):
		"""
		Returns a human-readable error string if DLL loading failed, else None.
		"""
...
def __test_cba4():
	...
	devices = cba4.scan()
	print("Found " + str(len(devices) if devices else 0) + " devices.")
